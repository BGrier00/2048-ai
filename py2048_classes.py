...
############################ some additional useful functions
    ...
    def possible_moves(self):
        ...
        values = state[0] + state[1] + state[2] + state[3]
        ...
        for ind, val in enumerate(values):
            ...
            # LEFT
            if missing[0] and col > 0:
                left = values[ind - 1]
                if left == val or left is None:
                    poss.append("LEFT")
                    if len(poss) == 4:
                        return poss
                    missing[0] = False
            ...
            # RIGHT
            if missing[1] and col < 3:
                right = values[ind + 1]
                if right == val or right is None:
                    poss.append("RIGHT")
                    if len(poss) == 4:
                        return poss
                    missing[1] = False
            ...
            # UP
            if missing[2] and row > 0:
                up = values[ind - 4]
                if up == val or up is None:
                    poss.append("UP")
                    if len(poss) == 4:
                        return poss
                    missing[2] = False
            ...
            # DOWN
            if missing[3] and row < 3:
                down = values[ind + 4]
                if down == val or down is None:
                    poss.append("DOWN")
                    if len(poss) == 4:
                        return poss
                    missing[3] = False
    ...
    def random_rollout(self, rounds):
        """Computes an expectation score by randomly playing the given
        number of `rounds` without altering `self`."""
        board_copy = deepcopy(self)
        for _ in range(rounds):
            if board_copy.is_board_full() and board_copy.possible_moves() == []:
                break
            random_move = random.choice(board_copy.possible_moves())
